...
| Qwen3-TTS (`Qwen3TTSTalker`) | Supported (deterministic inference) | `qwen3_tts_h200_dp2.yaml` | `model._decode_feedback_embedding.weight` (per-step decode staging) | 381 registered tensors (1.75 GiB) |
...
Validation update for #1401 Part 2: at code revision [`cd45a47`](https://github.com/sgl-project/sglang-omni/commit/cd45a47a1838017c89fb2178f167aac0cd7412a3), Qwen3-TTS passed `N=2`, `WEIGHT_SHARE=1` health, MPS attachment, concurrent byte-identity against the single-replica baseline, and clean teardown on H200. Its byte-identity qualification applies specifically to `enable_deterministic_inference: true`; default-mode inference was not qualified for byte identity.
